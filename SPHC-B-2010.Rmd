...
dates_all %>%
  ...
  bind_rows( summarise( ., year = "Total", n_deaths = sum( n_deaths ) ) ) # no death events in 2010
```
...
table( d_2010_complete$registered_year, useNA = "always" )
n_miss( d_2010_complete$lopnr )
```
...
summary( d_diagnosis_2010 )
nrow( d_diagnosis_2010 ) == length( unique( d_diagnosis_2010$lopnr ) ) # TRUE
```
...
```{r}
d_2010_complete_final <- d_2010_complete %>%
  left_join( atc_2010_summary_wide, by = "lopnr" ) %>%
  left_join( d_diagnosis_2010, by = "lopnr" ) %>%
  mutate(
    across(
...
summary( d_2010_complete_final )

# introduce missing data
d_2010_complete_final <- d_2010_complete_final %>%
...
overall_prevalence_2010_by_sex <- Reduce( function( x, y ){
  merge( x, y, by = "sex" )
}, 
results_list ) %>%
  mutate( sexual_identity = "Stockholm County" ) %>%
...
writexl::write_xlsx( overall_prevalence_2010_by_sex, "results_output/results_cc/overall_prevalence_2010_by_sex_cc.xlsx" )
...
# crude analysis (MALE)
levels( d_2010_complete_final$sex )
...
d_2010_incomplete <- d_2010_complete_final %>%
  filter( !is.na( lopnr ) ) %>%
...
miss_var_summary( d_2010_incomplete ) # 3.8% in sexual_identity_2010
```
...
fml_imp_2010 <- 
  sexual_identity_2010 + suicidal_thoughts + suicide_attempts + psychological_distress + 
  education + anxiety + depression + stress_related + used_N05B + used_N06A ~ 1 + 
  age + country_of_birth + income + ( 1 | weight_strata )

# perform a "dry run"
imp_ini_2010_by_sex <- list()

for ( sx in c( "Female", "Male" ) ) {
  
  imp_ini_2010_by_sex[[ sx ]] <- jomoImpute(
    data = d_2010_incomplete %>%
      filter( sex == sx ),
...
summary( imp_ini_2010_by_sex[[ "Female" ]] )
summary( imp_ini_2010_by_sex[[ "Male" ]] )


# sex-specific imputation
imp_2010_by_sex <- list()

for ( sx in c( "Female", "Male" ) ) {
  
  imp_data <- d_2010_incomplete %>%
    filter( sex == sx )
  
  imp_2010_by_sex[[ sx ]] <- jomoImpute(
    data = imp_data,
    formula = fml_imp_2010,
    random.L1 = "full",
    n.burn = 2000,
...
saveRDS(
  imp_2010_by_sex,
  file = "/Volumes/projects/LGBT Project data/Multiple Imputation/mental_health_project/imp_2010_by_sex.rds"
  )

# imp_2010_by_sex <- readRDS( "/Volumes/projects/LGBT Project data/Multiple Imputation/mental_health_project/imp_2010_by_sex.rds" )

summary( imp_2010_by_sex [[ "Female" ]] )
summary( imp_2010_by_sex [[ "Male" ]] )
plot( imp_2010_by_sex [[ "Female" ]], trace = "all", print = "beta" )
plot( imp_2010_by_sex [[ "Male" ]], trace = "all", print = "beta" )

implist_2010_female <- mitmlComplete(
  imp_2010_by_sex[[ "Female" ]],
  print = "all"
  )

implist_2010_male <- mitmlComplete(
  imp_2010_by_sex[[ "Male" ]],
  print = "all"
  )

implist_2010_all <- Map(
  rbind,
  implist_2010_female,
  implist_2010_male
  )

class( implist_2010_all )
length( implist_2010_all )

implist_2010_all[[ 1 ]] %>%
  count( sex )
summary( implist_2010_all[[ 1 ]] )
```

#### 5.3. Validate imputed data
```{r}
# plot imputed variables
d_2010_incomplete$imputation <- "0"
all_data_2010 <- bind_rows( d_2010_incomplete,
                            bind_rows( implist_2010_all, .id = "imputation" ) ) # merge datasets
...
# create survey design
design_imp_2010 <- svydesign( ids = ~ 1,
                              strata = ~ sampling_strata_region,
                              weights = ~ calibrated_weight,
                              fpc = ~ no.of.population,
                              data = imputationList( implist_2010_all )
                              ) # without pre-calibration

design_imp_2010 <- subset( design_imp_2010, age <= 29 )
class( design_imp_2010 )
...
##### 5.4.2. Prevalence
```{r}
overall_prev_2010 <- cal_prev_imp(
  variables_list = outcome_list,
  design = design_imp_2010,
  year = 2010,
  group_var = "sexual_identity_2010",
  subgroup = FALSE
  )

prev_2010_by_sex <- cal_prev_imp(
  variables_list = outcome_list,
  design = design_imp_2010,
  year = 2010,
  group_var = "sexual_identity_2010 + sex",
...
writexl::write_xlsx( overall_prev_2010, "results_output/results_mi/overall_prevalence_2010_imp.xlsx" )
writexl::write_xlsx( prev_2010_by_sex, "results_output/results_mi/prevalence_2010_by_sex_imp.xlsx" )
```
...
writexl::write_xlsx( pr_2010, "results_output/results_mi/pr_2010_imp.xlsx" )
...
for ( sx in sex_levels ) {
  ...
  design_sex <- subset( design_imp_2010, sex == sx )
...
writexl::write_xlsx( pr_2010_by_sex, "results_output/results_mi/pr_2010_by_sex_imp.xlsx" )
```
...
writexl::write_xlsx( pd_2010, "results_output/results_mi/pd_2010_imp.xlsx" )
...
writexl::write_xlsx( pd_2010_by_sex, "results_output/results_mi/pd_2010_by_sex_imp.xlsx" )
```