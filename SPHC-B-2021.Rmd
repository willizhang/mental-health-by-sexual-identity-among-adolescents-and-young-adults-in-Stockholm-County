...
included_year_ids <- move_in_before_year_ids # among those who had any migration event
...
eligible_dates %>%
  filter( lopnr %in% excluded_year_ids,
          date >= followup_start ) %>%
  count( event_type ) # 14 death events in 2021

# for those who had a death event in 2021, their prevalence outcome data will be treated as missing values, unless they experienced the outcome in 2021; their outcome data (i.e., prevalence data before death) will be imputed during multiple imputation
```
...
length( intersect( atc_2021_summary_wide$lopnr, excluded_year_ids ) ) # 6 excluded participants had prescription data, so their prevalence data will be treated as observed
...
reincluded_year_ids_N05B <- atc_2021_summary_wide %>%
  filter( lopnr %in% excluded_year_ids,
          used_N05B == "Yes" ) %>%
  pull( lopnr ) # 4 had dispensed prescription of anxiolytics
```
