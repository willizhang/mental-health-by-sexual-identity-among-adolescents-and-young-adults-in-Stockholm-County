...
table( d_2021_complete$registered_year, useNA = "always" )
n_miss( d_2021_complete$lopnr )
```
...
nrow( d_diagnosis_2021 ) == length( unique( d_diagnosis_2021$lopnr ) ) # TRUE
```
...
```{r}
d_2021_complete_final <- d_2021_complete %>%
  left_join( atc_2021_summary_wide, by = "lopnr" ) %>%
  left_join( d_diagnosis_2021, by = "lopnr" ) %>%
  mutate(
    across(
...
summary( d_2021_complete_final )

# introduce missing data
d_2021_complete_final <- d_2021_complete_final %>%
...
# crude analysis (MALE)
levels( d_2021_complete_final$sex )
...
miss_var_summary( d_2021_incomplete ) # 3.5% in psychological_distress and 2.2% in sexual_identity_2021
```
...
fml_imp_2021 <- 
  psychological_distress + sexual_identity_2021 + education + suicidal_thoughts + suicide_attempts + 
  income + used_N05B + depression + anxiety + used_N06A + stress_related ~ 1 + 
  age + country_of_birth + ( 1 | weight_strata )

# perform a "dry run"
imp_ini_2021_by_sex <- list()

for ( sx in c( "Female", "Male" ) ) {
  
  imp_ini_2021_by_sex[[ sx ]] <- jomoImpute(
    data = d_2021_incomplete %>%
      filter( sex == sx ),
...
summary( imp_ini_2021_by_sex[[ "Female" ]] )
summary( imp_ini_2021_by_sex[[ "Male" ]] )


# sex-specific imputation
imp_2021_by_sex <- list()

for ( sx in c( "Female", "Male" ) ) {
  
  imp_data <- d_2021_incomplete %>%
    filter( sex == sx )
  
  imp_2021_by_sex[[ sx ]] <- jomoImpute(
    data = imp_data,
    formula = fml_imp_2021,
    random.L1 = "full",
    n.burn = 2000,
...
saveRDS(
  imp_2021_by_sex,
  file = "/Volumes/projects/LGBT Project data/Multiple Imputation/mental_health_project/imp_2021_by_sex.rds"
)

# imp_2021_by_sex <- readRDS( "/Volumes/projects/LGBT Project data/Multiple Imputation/mental_health_project/imp_2021_by_sex.rds" )

summary( imp_2021_by_sex [[ "Female" ]] )
summary( imp_2021_by_sex [[ "Male" ]] )
plot( imp_2021_by_sex [[ "Female" ]], trace = "all", print = "beta" )
plot( imp_2021_by_sex [[ "Male" ]], trace = "all", print = "beta" )

implist_2021_female <- mitmlComplete(
  imp_2021_by_sex[[ "Female" ]],
  print = "all"
)

implist_2021_male <- mitmlComplete(
  imp_2021_by_sex[[ "Male" ]],
  print = "all"
)

implist_2021_all <- Map(
  rbind,
  implist_2021_female,
  implist_2021_male
)

class( implist_2021_all )
length( implist_2021_all )

implist_2021_all[[ 1 ]] %>%
  count( sex )
summary( implist_2021_all[[ 1 ]] )
```

#### 5.3. Validate imputed data
```{r}
# plot imputed variables
d_2021_incomplete$imputation <- "0"
all_data_2021 <- bind_rows( d_2021_incomplete,
                            bind_rows( implist_2021_all, .id = "imputation" ) ) # merge datasets
...
# create survey design
design_imp_2021 <- svydesign( ids = ~ 1,
                              strata = ~ sampling_strata_region,
                              weights = ~ calibrated_weight,
                              fpc = ~ no.of.population,
                              data = imputationList( implist_2021_all )
) # without pre-calibration

design_imp_2021 <- subset( design_imp_2021, age <= 29 )
class( design_imp_2021 )
...
##### 5.4.2. Prevalence
```{r}
overall_prev_2021 <- cal_prev_imp(
  variables_list = outcome_list,
  design = design_imp_2021,
  year = 2021,
  group_var = "sexual_identity_2021",
  subgroup = FALSE
)

prev_2021_by_sex <- cal_prev_imp(
  variables_list = outcome_list,
  design = design_imp_2021,
  year = 2021,
  group_var = "sexual_identity_2021 + sex",
...
writexl::write_xlsx( overall_prev_2021, "results_output/results_mi/overall_prevalence_2021_imp.xlsx" )
writexl::write_xlsx( prev_2021_by_sex, "results_output/results_mi/prevalence_2021_by_sex_imp.xlsx" )
```
...
for ( var in outcome_list ) {
  ...
  fml_adjusted <- as.formula(
    paste0(
      "I(", var$variable, " == '", var$condition, "') ~ ",
      "sexual_identity_2021 + age + sex + country_of_birth"
    )
  )
...
writexl::write_xlsx( pr_2021, "results_output/results_mi/pr_2021_imp.xlsx" )
...
for ( sx in sex_levels ) {
  ...
  design_sex <- subset( design_imp_2021, sex == sx )
  ...
  for ( var in outcome_list ) {
    ...
    fml_adjusted <- as.formula(
      paste0(
        "I(", var$variable, " == '", var$condition, "') ~ ",
        "sexual_identity_2021 + age + country_of_birth"
      )
    )
...
writexl::write_xlsx( pr_2021_by_sex, "results_output/results_mi/pr_2021_by_sex_imp.xlsx" )
```
...
writexl::write_xlsx( pd_2021, "results_output/results_mi/pd_2021_imp.xlsx" )
...
writexl::write_xlsx( pd_2021_by_sex, "results_output/results_mi/pd_2021_by_sex_imp.xlsx" )
```